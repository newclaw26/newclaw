...
async function launchNewClawElectron(
  homeDir: string,
  userDataDir: string,
  ...
  return await electron.launch({
    ...
    env: {
      ...
      NEWCLAW_E2E: '1',
      NEWCLAW_USER_DATA_DIR: userDataDir,
      ...(options.skipSetup ? { NEWCLAW_E2E_SKIP_SETUP: '1' } : {}),
      NEWCLAW_PORT_NEWCLAW_HOST_API: String(hostApiPort),
    },
    timeout: 90_000,
  });
}
...
export const test = base.extend<ElectronFixtures>({
  homeDir: async ({ browserName: _browserName }, provideHomeDir) => {
    const homeDir = await mkdtemp(join(tmpdir(), 'newclaw-e2e-home-'));
  ...
  userDataDir: async ({ browserName: _browserName }, provideUserDataDir) => {
    const userDataDir = await mkdtemp(join(tmpdir(), 'newclaw-e2e-user-data-'));
    try {
  ...
  launchElectronApp: async ({ homeDir, userDataDir }, provideLauncher) => {
    await provideLauncher(async (options?: LaunchElectronOptions) => await launchNewClawElectron(homeDir, userDataDir, options));
  },
