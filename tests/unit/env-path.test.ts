...
describe('env-path', () => {
  ...
  it('prepends entry with Windows delimiter', () => {
    setPlatform('win32');
    const next = prependPathEntry({ Path: 'C:\\Windows\\System32' }, 'D:\\newclaw\\resources\\bin');
    expect(next.path).toBe('D:\\newclaw\\resources\\bin;C:\\Windows\\System32');
    expect(next.env.Path).toBe('D:\\newclaw\\resources\\bin;C:\\Windows\\System32');
  });

  it('prepends entry with POSIX delimiter', () => {
    setPlatform('linux');
    const next = prependPathEntry({ PATH: '/usr/bin:/bin' }, '/opt/newclaw/bin');
    expect(next.path).toBe('/opt/newclaw/bin:/usr/bin:/bin');
    expect(next.env.PATH).toBe('/opt/newclaw/bin:/usr/bin:/bin');
  });
});
