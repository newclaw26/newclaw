...
const testOpenClawConfigDir = join(tmpdir(), 'newclaw-tests', 'channel-routes-openclaw');
...
describe('handleChannelRoutes', () => {
  ...
  it('reports healthy running multi-account channels as connected', async () => {
    ...
    const handled = await handleChannelRoutes(
      ...
      {} as ServerResponse,
      new URL('http://127.0.0.1:13220/api/channels/accounts'),
      {
        gatewayManager: {
          rpc,
  ...
  it('lists known QQ Bot targets for a configured account', async () => {
    ...
    const handled = await handleChannelRoutes(
      { method: 'GET' } as IncomingMessage,
      {} as ServerResponse,
      new URL('http://127.0.0.1:13220/api/channels/targets?channelType=qqbot&accountId=default'),
      {
        gatewayManager: {
          rpc: vi.fn(),
          getStatus: () => ({ state: 'running' }),
  ...
  it('lists Feishu targets for a configured account', async () => {
    ...
    const handled = await handleChannelRoutes(
      { method: 'GET' } as IncomingMessage,
      {} as ServerResponse,
      new URL('http://127.0.0.1:13220/api/channels/targets?channelType=feishu&accountId=default'),
      {
        gatewayManager: {
          rpc: vi.fn(),
          getStatus: () => ({ state: 'running' }),
  ...
  it('lists WeCom targets from reqid cache and session history', async () => {
    ...
    const handled = await handleChannelRoutes(
      { method: 'GET' } as IncomingMessage,
      {} as ServerResponse,
      new URL('http://127.0.0.1:13220/api/channels/targets?channelType=wecom&accountId=default'),
      {
        gatewayManager: {
          rpc: vi.fn(),
          getStatus: () => ({ state: 'running' }),
  ...
  it('lists DingTalk targets from session history', async () => {
    ...
    const handled = await handleChannelRoutes(
      { method: 'GET' } as IncomingMessage,
      {} as ServerResponse,
      new URL('http://127.0.0.1:13220/api/channels/targets?channelType=dingtalk&accountId=default'),
      {
        gatewayManager: {
          rpc: vi.fn(),
          getStatus: () => ({ state: 'running' }),
  ...
  it('lists WeChat targets from session history via the UI alias', async () => {
    ...
    const handled = await handleChannelRoutes(
      { method: 'GET' } as IncomingMessage,
      {} as ServerResponse,
      new URL('http://127.0.0.1:13220/api/channels/targets?channelType=wechat&accountId=wechat-bot'),
      {
        gatewayManager: {
          rpc: vi.fn(),
          getStatus: () => ({ state: 'running' }),
