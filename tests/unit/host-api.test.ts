...
describe('host-api', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    window.localStorage.removeItem('newclaw:allow-localhost-fallback');
  });
  ...
  it('falls back to browser fetch when hostapi handler is not registered', async () => {
    ...
    window.localStorage.setItem('newclaw:allow-localhost-fallback', '1');
