...
export function Settings() {
    ...
    (async () => {
      try {
        ...
        if (result.success && result.command) {
          ...
          setOpenclawCliError(result.error || 'NewClaw CLI unavailable');
        }
      } catch (error) {
        if (cancelled) return;
        setOpenclawCliCommand('');
  ...
  useEffect(() => {
    const unsubscribe = window.electron.ipcRenderer.on(
      'newclaw:cli-installed',
      (...args: unknown[]) => {
        const installedPath = typeof args[0] === 'string' ? args[0] : '';
        toast.success(`newclaw CLI installed at ${installedPath}`);
      },
    );
    return () => { unsubscribe?.(); };
  }, []);
            ...
            <div className="space-y-3 text-[14px] text-muted-foreground">
              ...
              <div className="flex gap-4 pt-3">
                <Button
                  ...
                  onClick={() => window.electron.openExternal('https://newclaw.app')}
                >
                  {t('about.docs')}
                </Button>
                <Button
                  variant="link"
                  className="h-auto p-0 text-[14px] text-blue-500 hover:text-blue-600 font-medium"
                  onClick={() => window.electron.openExternal('https://github.com/newclaw-ai/NewClaw')}
                >
                  {t('about.github')}
                </Button>
                <Button
