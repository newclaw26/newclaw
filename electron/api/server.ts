 ...
 * if they can reach 127.0.0.1:13220 (the CORS wildcard alone is not
...
export function startHostApiServer(ctx: HostApiContext, port = getPort('NEWCLAW_HOST_API')): Server {
  ...
  server.on('error', (error: NodeJS.ErrnoException) => {
    if (error.code === 'EACCES' || error.code === 'EADDRINUSE') {
      logger.error(
        ...
        `Set NEWCLAW_PORT_NEWCLAW_HOST_API env var to override the default port.`,
      );
    } else {
