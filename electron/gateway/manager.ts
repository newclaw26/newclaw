 ...
 * Manages the NewClaw Gateway process lifecycle
 */
...
/**
 * Gateway Manager
 * Handles starting, stopping, and communicating with the NewClaw Gateway
 */
export class GatewayManager extends EventEmitter {
  ...
  private async initDeviceIdentity(): Promise<void> {
    ...
    try {
      const identityPath = path.join(app.getPath('userData'), 'newclaw-device-identity.json');
   ...
   * Uses NewClaw protocol format: { type: "req", id: "...", method: "...", params: {...} }
   */
  async rpc<T>(method: string, params?: unknown, timeoutMs = 30000): Promise<T> {
    return new Promise((resolve, reject) => {
      ...
      // Send request using NewClaw protocol format
      const request = {
        type: 'req',
        id,
        method,
   ...
   * NewClaw Gateway doesn't have an HTTP /health endpoint
   */
  ...
  /**
   * Start Gateway process
   * Uses NewClaw npm package from node_modules (dev) or resources (production)
   */
  ...
  private handleMessage(message: unknown): void {
    ...
    // Handle NewClaw protocol response format: { type: "res", id: "...", ok: true/false, ... }
    ...
    // Handle NewClaw protocol event format: { type: "event", event: "...", payload: {...} }
