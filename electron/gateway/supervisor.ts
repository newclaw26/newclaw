...
export async function runOpenClawDoctorRepair(): Promise<boolean> {
  ...
  if (!existsSync(entryScript)) {
    logger.error(`Cannot run NewClaw doctor repair: entry script not found at ${entryScript}`);
    return false;
  }
  ...
  logger.info(
    `Running NewClaw doctor repair (entry="${entryScript}", args="${doctorArgs.join(' ')}", cwd="${openclawDir}", bundledBin=${binPathExists ? 'yes' : 'no'})`,
  );
  ...
  return await new Promise<boolean>((resolve) => {
    ...
    const timeout = setTimeout(() => {
      logger.error('NewClaw doctor repair timed out after 120000ms');
      try {
        child.kill();
      } catch {
        // ignore
    ...
    child.on('error', (err) => {
      clearTimeout(timeout);
      logger.error('Failed to spawn NewClaw doctor repair process:', err);
      finish(false);
    });
    ...
    child.on('exit', (code: number) => {
      clearTimeout(timeout);
      if (code === 0) {
        logger.info('NewClaw doctor repair completed successfully');
        finish(true);
        return;
      }
      logger.warn(`NewClaw doctor repair exited (code=${code})`);
      finish(false);
    });
  });
}
