...
(function () {
  ...
  if (globalThis.__newclawFetchPatched) return;
  globalThis.__newclawFetchPatched = true;

  globalThis.fetch = function newclawFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      delete flat['x-title'];
      delete flat['X-Title'];
      flat['HTTP-Referer'] = 'https://newclaw.app';
      flat['X-Title'] = 'NewClaw';
      init.headers = flat;
    }
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__newclawPatched) {
        cp.__newclawPatched = true;
