...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the NewClaw chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      const gatewayInput = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via NewClaw WebSocket chat events,
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[newclaw-validate] validating provider type: ${providerType}`);
