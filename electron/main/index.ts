...
import { ensureNewClawContext, repairNewClawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
import { registerCodeEngineIpcHandlers, initCodeEngine, shutdownCodeEngine } from '../code-engine/ipc-handlers';

const WINDOWS_APP_USER_MODEL_ID = 'app.newclaw.desktop';
const isE2EMode = process.env.NEWCLAW_E2E === '1';
const requestedUserDataDir = process.env.NEWCLAW_USER_DATA_DIR?.trim();

if (isE2EMode && requestedUserDataDir) {
  app.setPath('userData', requestedUserDataDir);
}

// macOS 26 透明玻璃界面需要 GPU 加速来支持 vibrancy 和 backdrop-filter。
// 仅在 Windows/Linux 上禁用硬件加速以保持稳定性。
if (process.platform !== 'darwin') {
  app.disableHardwareAcceleration();
}

// On Linux, set CHROME_DESKTOP so Chromium can find the correct .desktop file.
// On Wayland this maps the running window to newclaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('newclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[NewClaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[NewClaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[NewClaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
function createWindow(): BrowserWindow {
  ...
  const shouldSkipSetupForE2E = process.env.NEWCLAW_E2E_SKIP_SETUP === '1';
  ...
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false,
      webviewTag: true, // Enable <webview> for embedding NewClaw Control UI
    },
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== NewClaw Application Starting ===');
  logger.debug(
  ...
  // Override security headers ONLY for the NewClaw Gateway Control UI.
  ...
  // Register Code Engine IPC handlers and start engine
  registerCodeEngineIpcHandlers(window);
  void initCodeEngine().catch((error) => {
    logger.warn('Code Engine init failed (non-fatal):', error);
  });

  // Note: Auto-check for updates is driven by the renderer (update store init)
  // so it respects the user's "Auto-check for updates" setting.

  // Repair any bootstrap files that only contain NewClaw markers (no NewClaw
  // template content). This fixes a race condition where ensureNewClawContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairNewClawOnlyBootstrapFiles().catch((error) => {
  ...
  // Pre-deploy/upgrade bundled NewClaw plugins (dingtalk, wecom, feishu, wechat)
  // to ~/.openclaw/extensions/ so they are always up-to-date after an app update.
  // Note: qqbot was moved to a built-in channel in NewClaw 3.31.
  if (!isE2EMode) {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running' && !isE2EMode) {
      void ensureNewClawContext().catch((error) => {
        logger.warn('Failed to re-merge NewClaw context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge NewClaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureNewClawContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureNewClawContext().catch((error) => {
      logger.warn('Failed to merge NewClaw context into workspace:', error);
    });
  }

  // Auto-install newclaw CLI and shell completions (non-blocking).
  if (!isE2EMode) {
    void autoInstallCliIfNeeded((installedPath) => {
      mainWindow?.webContents.send('newclaw:cli-installed', installedPath);
    }).then(() => {
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second NewClaw instance detected; redirecting to the existing window');
