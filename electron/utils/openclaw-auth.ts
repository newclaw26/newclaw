 ...
 * Used by NewClaw to seed the provider store when it's empty but providers are
...
/**
 * Write the NewClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged NewClaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  NewClaw sets
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // for NewClaw to properly integrate with its updated tool system.
    ...
    // NewClaw is a local desktop app where the user is the trusted operator.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for NewClaw desktop');
    }
    ...
    // but NewClaw historically stored them only under `channels.<type>.accounts.default`.
