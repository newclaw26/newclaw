...
import { getNewClawConfigDir } from './paths';
...
const LOCAL_GEMINI_DIR = join(getNewClawConfigDir(), 'gemini-cli');
    ...
    const server = createServer((req, res) => {
      try {
        ...
        if (state !== params.expectedState) {
          ...
          res.end(
            "<!doctype html><html><head><meta charset='utf-8'/></head><body><h2>Session expired</h2><p>This authorization link is from a previous attempt. Please go back to NewClaw and try again.</p></body></html>",
          );
          return;
        }
        ...
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.end(
          "<!doctype html><html><head><meta charset='utf-8'/></head><body><h2>Gemini CLI OAuth complete</h2><p>You can close this window and return to NewClaw.</p></body></html>",
        );
...
export async function loginGeminiCliOAuth(
  ...
  if (ctx.isRemote) {
    throw new Error('Remote/manual Gemini OAuth is not implemented in NewClaw yet.');
  }
