...
function appendDoctorOutput(
  ...
  logger.warn(
    `NewClaw doctor ${stream} exceeded ${MAX_DOCTOR_OUTPUT_BYTES} bytes; truncating additional output`,
  );
...
async function runDoctorCommandWithArgs(
  ...
  const command = `newclaw ${args.join(' ')}`;
  const startedAt = Date.now();

  if (!existsSync(entryScript)) {
    const error = `NewClaw entry script not found at ${entryScript}`;
    logger.error(`Cannot run NewClaw doctor: ${error}`);
    return {
      mode,
      success: false,
      exitCode: null,
  ...
  logger.info(
    `Running NewClaw doctor (mode=${mode}, entry="${entryScript}", args="${args.join(' ')}", cwd="${openclawDir}", bundledBin=${binPathExists ? 'yes' : 'no'})`,
  );
  ...
  return await new Promise<OpenClawDoctorResult>((resolve) => {
    ...
    const timeout = setTimeout(() => {
      logger.error(`NewClaw doctor timed out after ${OPENCLAW_DOCTOR_TIMEOUT_MS}ms`);
      try {
        child.kill();
      } catch {
        // ignore
    ...
    child.on('error', (error) => {
      ...
      logger.error('Failed to spawn NewClaw doctor process:', error);
      finish({
        mode,
        success: false,
        exitCode: null,
    ...
    child.on('exit', (code) => {
      clearTimeout(timeout);
      logger.info(`NewClaw doctor exited with code ${code ?? 'null'}`);
      finish({
        mode,
        success: code === 0,
        exitCode: code,
