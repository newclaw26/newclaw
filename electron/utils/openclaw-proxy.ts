...
interface SyncProxyOptions {
   ...
   * currently disabled in NewClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync NewClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because NewClaw proxy is disabled and preserve mode is enabled');
      return;
    }
