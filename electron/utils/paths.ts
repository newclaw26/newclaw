...
function getElectronApp() {
  ...
  const fallbackUserData = process.env.NEWCLAW_USER_DATA_DIR?.trim() || join(homedir(), '.newclaw');
...
/**
 * Get NewClaw config directory
 */
export function getNewClawConfigDir(): string {
  return join(homedir(), '.newclaw');
}

/**
 * Get NewClaw logs directory
 */
