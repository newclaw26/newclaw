...
import { getNewClawProviderStore } from '../services/providers/store-instance';
import {
...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getNewClawProviderStore();
 ...
 * Also synchronizes NewClaw local provider list with OpenClaw's actual config.
 */
...
> {
  ...
  for (const provider of providers) {
    ...
    // we should remove it from NewClaw UI to stay consistent.
