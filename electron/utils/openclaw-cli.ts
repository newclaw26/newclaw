/**
 * NewClaw CLI utilities — cross-platform auto-install
 */
...
export function getOpenClawCliCommand(): string {
  ...
  if (platform === 'darwin' || platform === 'linux') {
    const localBinPath = join(homedir(), '.local', 'bin', 'newclaw');
  ...
  if (platform === 'linux') {
    if (existsSync('/usr/local/bin/newclaw')) {
      return '/usr/local/bin/newclaw';
    }
  }
  ...
  if (!app.isPackaged) {
    ...
    const binName = platform === 'win32' ? 'newclaw.cmd' : 'newclaw';
  ...
  if (app.isPackaged) {
    if (platform === 'win32') {
      ...
      const cmdPath = join(cliDir, 'newclaw.cmd');
...
function getPackagedCliWrapperPath(): string | null {
  ...
  if (platform === 'darwin' || platform === 'linux') {
    const wrapper = join(process.resourcesPath, 'cli', 'newclaw');
    return existsSync(wrapper) ? wrapper : null;
  }
  if (platform === 'win32') {
    const wrapper = join(process.resourcesPath, 'cli', 'newclaw.cmd');
    return existsSync(wrapper) ? wrapper : null;
  }
  return null;
}
...
function getCliTargetPath(): string {
  return join(homedir(), '.local', 'bin', 'newclaw');
}
...
export async function installOpenClawCli(): Promise<{
  ...
  try {
    ...
    logger.info(`NewClaw CLI symlink created: ${target} -> ${wrapperSrc}`);
    return { success: true, path: target };
  } catch (error) {
    logger.error('Failed to install NewClaw CLI:', error);
...
function isCliInstalled(): boolean {
  ...
  if (platform === 'linux' && existsSync('/usr/local/bin/newclaw')) return true;
