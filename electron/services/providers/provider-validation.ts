...
function logValidationStatus(provider: string, status: number): void {
  console.log(`[newclaw-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  console.log(
    `[newclaw-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`,
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (shouldFallbackFromModelsProbe(modelsResult)) {
    console.log(
      `[newclaw-validate] ${providerType} /models returned ${modelsResult.status}, falling back to ${apiProtocol} probe`,
    );
    if (apiProtocol === 'openai-responses') {
...
async function validateAnthropicHeaderKey(
  ...
  if (
    ...
    console.log(
      `[newclaw-validate] ${providerType} /models returned error, falling back to /messages probe`,
    );
